...
import pandas as pd
import numpy as np
import joblib, os, re, tldextract, shap, time, datetime, warnings, csv
...
def predict_url_simple(url):
    ...
    if trust_index >= 0.6: risk="Safe"
    elif trust_index >= 0.4: risk="Suspicious"
    else: risk="Phishing"
...
def explain_url_shap(X_hybrid):
    try:
        ...
        ax.set_title("Top 10 Influencing Features (SHAP)")
        plt.tight_layout()
        st.pyplot(fig)
        return "shap"
    except Exception as e:
        st.error(f"⚠️ SHAP failed: {e}")
        st.info("Switching to fallback feature importances.")
        explain_fallback()
        return "fallback"
...
# === Streamlit UI =========================================
# ==========================================================
st.set_page_config(page_title="PhishTriage – Explainable Detector (Report)", page_icon="🧠", layout="centered")
st.title("🧠 PhishTriage – Explainable Phishing URL Detector with Reports")
st.markdown("Enter a URL, analyze it, and download the explainability report.")

url_input = st.text_input("🔗 Enter URL:", placeholder="https://example.com")
analyze_btn = st.button("Analyze & Explain")

if analyze_btn and url_input.strip():
    with st.spinner("Running model + explainability..."):
    ...
    st.markdown("---")
    st.subheader("📊 Explanation: Feature Influence")
    mode = explain_url_shap(X_hybrid)
    log_result(url_input, prob, rule_score, trust_index, risk, rules, mode)
    st.success("✅ Explanation & log entry saved.")
else:
...
if os.path.exists(log_path):
    ...
    st.download_button("⬇️ Download Report as CSV", csv_data, "explainability_report.csv", "text/csv")
else:
    st.warning("No logs available yet. Run at least one analysis first.")
