...
import pandas as pd
import numpy as np
import joblib, os, re, tldextract, shap, time, datetime
...
def explain_url_shap(X_hybrid):
    ...
    vals = shap_values[1][0]   # class 1 = phishing
    abs_vals = np.abs(vals)
    top_idx = np.argsort(abs_vals)[-10:][::-1]  # top 10 influential
...
# === Streamlit UI =========================================
# ==========================================================
st.set_page_config(page_title="PhishTriage – Explainable Detector", page_icon="🧠", layout="centered")
st.title("🧠 PhishTriage – Explainable Phishing URL Detector")
st.markdown("Enter a URL below and click **Analyze & Explain** to view model reasoning.")
